...
(define-sort Time () Real) ; The installation step
...
(assert (installed |C=1| 0)) ; 11
(declare-const |D=10.3.1| Pkg) ; 12
(assert (installed |D=10.3.1| 0)) ; 13
(assert (depends |_VIRTUAL_=| (insert |B=3.2|(insert |B=3.0| nil)))) ; 14
(assert (depends |A=2.1| (insert |B=3.2|(insert |C=1| nil)))) ; 15
(assert (depends |A=2.1| (insert |D=10.3.1| nil))) ; 16
(assert (depends |C=1| (insert |D=10.3.1| nil))) ; 17
(assert (depends |D=10.3.1| (insert |C=1| nil))) ; 18
(assert (installed |_VIRTUAL_=| t-final)) ; 19
(check-sat) ; 20
(eval t-final) ; 21
(get-model)
 ; 22
...
; For all of the dependencies, at least one has to be installed
(define-fun-rec at-least-one-installed-of ((ps (List Pkg)) (t Time)) Bool
  (ite (= nil ps) false (or (and (installed (head ps) t) (installed (head ps) (- t 0))) (at-least-one-installed-of (tail ps) t))))
