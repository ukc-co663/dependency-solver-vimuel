...
(assert (installed |_VIRTUAL_=| t-final)) ; 30
(check-sat) ; 31
(eval t-final) ; 32
(get-model) ; 33
(exit) ; 34
...
; For all of the dependencies, at least one has to be installed
(define-fun-rec at-least-one-installed-of ((ps (List Pkg)) (t Time)) Bool
  (ite (= nil ps) false (or (installed (head ps) t) ) (at-least-one-installed-of (tail ps) t))))
